...
{
    uint32_t level;
    __asm volatile("rsr %0, intlevel" : "=a"(level));
    return level;
}
...
{
    uint32_t old_level;
    __asm volatile ("rsil %0, " XTSTR(XCHAL_EXCM_LEVEL) : "=a" (old_level));
    return old_level;
}
...
{
    __asm volatile ("wsr %0, ps; rsync" :: "a" (new_ps));
}
...
{
    uint32_t intenable;
    __asm volatile ("rsr %0, intenable" : "=a" (intenable));
    intenable |= unmask;
    __asm volatile ("wsr %0, intenable; esync" :: "a" (intenable));
}
...
{
    uint32_t interrupt;
    __asm volatile ("rsr %0, interrupt" : "=a" (interrupt));
    return interrupt;
}

static inline void _xt_clear_ints(uint32_t mask)
{
    __asm volatile ("wsr %0, intclear; esync" :: "a" (mask));
}
