...
//bool spi_set_settings(uint8_t bus, const spi_settings_t *s);
...
//uint32_t spi_get_frequency_div(uint8_t bus);
...
//            (FIELD2VAL(SPI_CLOCK_COUNT_NUM, SPI(bus).CLOCK) + 1);
//}
uint32_t spi_get_frequency_hz(uint8_t bus);
...
//spi_endianness_t spi_get_endianness(uint8_t bus);
...
//void spi_set_command(uint8_t bus,uint8_t bits, uint16_t data);
...
//void spi_set_address(uint8_t bus,uint8_t bits, uint32_t data);
...
//void spi_set_dummy_bits(uint8_t bus, uint8_t bits, bool pos);
...
//void spi_clear_address(uint8_t bus);
...
//void spi_clear_command(uint8_t bus);
...
//void spi_clear_dummy(uint8_t bus);
